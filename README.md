...
### Neon no Claude, ChatGPT e agentes de IA via Open Finance

Conecte sua conta do **Neon** ao Claude, ChatGPT e agentes de IA via Open Finance Brasil. Pergunte sobre saldos, extratos, faturas de cartão e investimentos do Neon em linguagem natural. Regulado pelo Banco Central, somente leitura.

- 🏦 **Neon via Open Finance Brasil**, regulado pelo Banco Central
...
**Manual** (se o deeplink não abrir): [claude.ai/customize/connectors](https://claude.ai/customize/connectors) → **+** → **Adicionar conector personalizado** → cole **Nome** `Neon MCP` e **URL** `https://api.mcp.ai/p_neon`.

### Cursor

[➕ Instalar Neon MCP no Cursor](cursor://anysphere.cursor-deeplink/mcp/install?name=neon&config=eyJ1cmwiOiJodHRwczovL2FwaS5tY3AuYWkvcF9uZW9uIn0=)

### VS Code (Copilot Chat)

[➕ Instalar Neon MCP no VS Code](vscode:mcp/install?name=neon&config=%7B%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fapi.mcp.ai%2Fp_neon%22%7D)
...
```
Quanto gastei no cartão do Neon esse mês?
Resuma minha fatura do Neon em aberto
Qual meu saldo na conta do Neon?
Como está minha carteira de investimentos no Neon?
```
...
**Como funciona?**
Via Open Finance Brasil (regulado pelo Banco Central). Você conecta sua conta Neon com consentimento explícito; a IA lê saldos, extratos, faturas e investimentos. Somente leitura.

**A IA pode movimentar dinheiro?**
Não. 100% leitura — Open Finance não permite movimentação por essa via.

**Só funciona com o Neon?**
Este MCP é focado no Neon. Para conectar vários bancos de uma vez, use o Banco MCP (api.mcp.ai/banco).
...
- 📧 [neon@mcp.ai](mailto:neon@mcp.ai)
- 🐛 [GitHub Issues](https://github.com/mcp-dir/neon-mcp/issues)
- 📄 [docs/](docs/)
...
MIT — veja [LICENSE](LICENSE). O servidor MCP em `api.mcp.ai/p_neon` é proprietário (hosted); este repositório (manifestos, docs, skills) é MIT.
