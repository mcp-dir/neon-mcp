...
### Neon in Claude, ChatGPT and AI agents via Open Finance

Connect your **Neon** account to Claude, ChatGPT and AI agents via Brazil's Open Finance. Ask about Neon balances, statements, credit-card bills and investments in natural language. Regulated by the Central Bank, read-only.

- 🏦 **Neon via Brazil's Open Finance**, regulated by the Central Bank
...
[➕ Open in Claude and connect](https://claude.ai/customize/connectors?modal=add-custom-connector&mcpName=Neon%20MCP&mcpServerUrl=https%3A%2F%2Fapi.mcp.ai%2Fp_neon)

Manual: [claude.ai/customize/connectors](https://claude.ai/customize/connectors) → **+** → **Add custom connector** → name `Neon MCP`, URL `https://api.mcp.ai/p_neon`.

### Cursor

[➕ Install in Cursor](cursor://anysphere.cursor-deeplink/mcp/install?name=neon&config=eyJ1cmwiOiJodHRwczovL2FwaS5tY3AuYWkvcF9uZW9uIn0=)

### VS Code (Copilot Chat)

[➕ Install in VS Code](vscode:mcp/install?name=neon&config=%7B%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fapi.mcp.ai%2Fp_neon%22%7D)
...
MIT — see [LICENSE](LICENSE). The MCP server at `api.mcp.ai/p_neon` is proprietary (hosted); this repo (manifests, docs, skills) is MIT.
